...
# Load observations(SP3)
# Generate collision trajectory "d" days in the future
# Select force model configuration
# Perform OD -> get optimized state and covariance matrix at t0
# Propagate the state for "d" + 0.5 (to not miss the collision)

#if using Patera2005:
    # Identify the TCA
    # Get the state at the TCA
    # Propagate the covariance matrix to TCA
    # Calculate the probability of collision

# is using Monte Carlo:
    # Diagonalize the covariance matrix of the two objects at t0
    # Generate perturbations from the diagonalized covariance matrix (centred around 0 with standrad deviation equal to the square root of the eigenvalues of the covariance matrix)
    # Generate new states to add the perturbations to the state at t0
