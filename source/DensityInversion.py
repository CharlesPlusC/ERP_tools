...
import os
from tools.utilities import utc_to_mjd, HCL_diff, get_satellite_info, pos_vel_from_orekit_ephem, EME2000_to_ITRF
...
def compute_gravitational_potential(r, phi, lambda_, degree, order, date):
    # Access the unnormalized spherical harmonics provider
    ...
    # Retrieve the unnormalized spherical harmonics for the given date
    harmonics = provider.onDate(date)
    # Constants for Earth's gravitational parameter and equatorial radius
    mu = provider.getMu()
    a = provider.getAe()
    # Initialize the potential
    V = 0.0
    # Pre-compute the associated Legendre polynomials for all n, m up to degree and order
    P_nm_all = associated_legendre_polynomials(degree, order, np.sin(phi))
    # Iterate over the degrees and orders to sum the series
    for n in range(degree + 1):
        for m in range(min(n, order) + 1):
            # Retrieve the unnormalized coefficients
            C_nm = harmonics.getNormalizedCnm(n,m)
            S_nm = harmonics.getNormalizedSnm(n,m)
            # Retrieve the specific Legendre polynomial value
            P_nm = P_nm_all[n, m]
            # Sectorial term calculation
            sectorial_term = (C_nm * np.cos(m * lambda_) + S_nm * np.sin(m * lambda_)) * P_nm
            # Accumulate the potential
            V += ((a / r) ** n) * sectorial_term
    # Finalize the potential calculation
    V = (mu / r) * V
    return V
...
def energy(ephemeris_df):
    ...
    # Calculate energy for each row from 2-body problem
    ephemeris_df['energy'] = 0.5 * (ephemeris_df['xv']**2 + ephemeris_df['yv']**2 + ephemeris_df['zv']**2) \
                             - 0.5 * omega_earth**2 * (ephemeris_df['x']**2 + ephemeris_df['y']**2) \
                             - mu / np.sqrt(ephemeris_df['x']**2 + ephemeris_df['y']**2 + ephemeris_df['z']**2)

    # add non-spherical terms
    # get r, phi, lamba, degree, order, date
    # then subtract the u non-spherical from the energy
...
def main():
    ...
    for sat_name in sat_names_to_test:
        ...
        # Plot the energy
        plt.figure()
        plt.plot(ephemeris_df['MJD'], ephemeris_df['energy'])
        plt.xlabel('MJD')
    ...
    latitudes = np.linspace(-np.pi / 2, np.pi / 2, 12)  # Reduced points for latitude
    longitudes = np.linspace(-np.pi, np.pi, 24)  # Reduced points for longitude
